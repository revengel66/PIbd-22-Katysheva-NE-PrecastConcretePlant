...
namespace PrecastConcretePlantListImplement.Implements
{
    public class OrderStorage : IOrderStorage
    {
        ...
        public List<OrderViewModel> GetFilteredList(OrderBindingModel model)
        {
            ...
            foreach (var order in source.Orders)
            {
                if (order.ReinforcedId == model.ReinforcedId)
                {
                    result.Add(CreateModel(order));
                }
            }
        ...
        private OrderViewModel CreateModel(Order order)
        {
            ...
            foreach (var document in source.Reinforceds)
            {
                if (document.Id == order.ReinforcedId)
                {
                    ReinforcedName = document.ReinforcedName;
                    break;
                }
